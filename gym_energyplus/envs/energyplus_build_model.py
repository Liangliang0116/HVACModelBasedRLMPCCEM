...
def build_ep_model(model_file, log_dir, config=None, verbose=False):
    ...
    if match('2ZoneDataCenterHVAC_wEconomizer_Temp_Fan.*', model_basename):
        model = EnergyPlusModel2ZoneDataCenterHVAC_wEconomizer_Temp_Fan(model_file=model_file, 
                                                                        log_dir=log_dir,
                                                                        config=config,
                                                                        verbose=verbose)
    elif match('2ZoneDataCenterHVAC_wEconomizer_Temp.*', model_basename):
        model = EnergyPlusModel2ZoneDataCenterHVAC_wEconomizer_Temp(model_file=model_file, 
                                                                    log_dir=log_dir,
                                                                    verbose=verbose)
    elif match('2ZoneDataCenterHVAC_wEconomizer.*', model_basename):
        model = EnergyPlusModel2ZoneDataCenterHVAC_wEconomizer(model_file=model_file, 
                                                               log_dir=log_dir, 
                                                               verbose=verbose)
    else:
