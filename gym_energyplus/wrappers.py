...
class EnergyPlusGradualActionWrapper(CostFnWrapper):
    """ 
    Normalize the actions of environment. 
    The action shape is normalized. 
    The observation space is set to be the concatenation of both observation 
    ...
    def action(self, action: np.ndarray) -> np.ndarray:
        """ Convert the action increment to the absolute action. 

        Args:
            action (np.ndarray): The action increment

        Returns:
            np.ndarray: The absolute action after clipping 
        """
...
class EnergyPlusSplitEpisodeWrapper(CostFnWrapper):
    """
    Break a super long episode env into small length episodes. Used for PPO TODO
...
class EnergyPlusObsWrapper(ObservationWrapper, CostFnWrapper):
    ...
    The temperature center will be set as the mean of the corresponding observations.
    The observation space will be normalized. 
